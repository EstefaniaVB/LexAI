...
def get_news_data():
    ...
    for outlet in news:
        try:
            news_data.append(python_tweets.get_user_timeline(screen_name=outlet, count=3,tweet_mode="extended"))
            print(outlet)
        except:
...
def get_news_data_filtered():
    data_new_filtered=[]
    for column in get_news_data():
        for row in get_news_data()[column]:
            id_new = row["id"]
            text = row["full_text"]
            try:
...
def get_politicians_data():
    ...
    for politic in politicians_1set:
        try:
            meps_data.append(python_tweets.get_user_timeline(screen_name=politic, count=2,tweet_mode="extended"))
            print(f"Downloading tweets from: ...{politic}")
        except:
            print(politic + " Not Found")
